...
def save_fullpage_screenshot(url, output_path):
    chrome_options = Options()
    chrome_options.add_argument("user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                                "AppleWebKit/537.36 (KHTML, like Gecko) "
                                "Chrome/90.0.4430.85 Safari/537.36")
    ...
    try:
        ...
        # 2. Get the page's layout metrics using the DevTools Protocol
        metrics = driver.execute_cdp_cmd("Page.getLayoutMetrics", {})
        
        # contentSize has the full size of the rendered page
        content_size = metrics["contentSize"]
        width = content_size["width"]
        height = content_size["height"]

        # 3. Resize the "window" to match the full content size
        ...
        # 4. Capture screenshot via DevTools
        result = driver.execute_cdp_cmd("Page.captureScreenshot", {
            "format": "png",
            "fromSurface": True
        })

        # 5. Decode the base64 screenshot string and save to file
...
if __name__ == '__main__':
    ...
    url = 'https://www.oddschecker.com/tennis/atp-monte-carlo/jordan-thompson-v-giovanni-mpetshi-perricard/winner'
    ...
    for x in range(300):
        ...
        print('Saving as screenshot.')
        save_fullpage_screenshot(url, 'screenshots/test_screenshot2.png')
